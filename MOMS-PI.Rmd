...
refTPt <- rownames(aldexrun)[which(abs(aldexrun$we.eBH) > 0.01)]
...
refTPt5 <- rownames(aldexrun)[which(abs(aldexrun$we.eBH) > 0.01)]
...
refTPt10 <- rownames(aldexrun)[which(abs(aldexrun$we.eBH) > 0.01)]
...
refTPt20 <- rownames(aldexrun)[which(abs(aldexrun$we.eBH) > 0.01)]
...
refTPt50 <- rownames(aldexrun)[which(abs(aldexrun$we.eBH) > 0.01)]
...
refTPw <- rownames(aldexrun)[which(abs(aldexrun$wi.eBH) > 0.01)]
...
refTPw5 <- rownames(aldexrun)[which(abs(aldexrun$wi.eBH) > 0.01)]
...
refTPw10 <- rownames(aldexrun)[which(abs(aldexrun$wi.eBH) > 0.01)]
...
refTPw20 <- rownames(aldexrun)[which(abs(aldexrun$wi.eBH) > 0.01)]
...
refTPw50 <- rownames(aldexrun)[which(abs(aldexrun$wi.eBH) > 0.01)]
...
# matrix of results
n.iter = 100
deseq2.test.results <- matrix(data=NA, ncol=5, nrow=n.iter)
colnames(deseq2.test.results) <- c("size", "TP", "FP","sens","spec")
for(i in 1:100){
size = 10
sub.condd<- c(rep("H", size), rep("BV", size))
sub.idsd <- c(sample(H.idd,size), sample(BV.idd,size))
sub.datad <- ko.virginia.filt[,sub.idsd]

#ddsd <- DESeqDataSetFromMatrix(countData = sub.datad, colData = sub.idsd, design= ~sub.condd)\

res2d <- DESeq (ddsd)
deseq2outd <- results(res2d)
sub.deseq <-rownames(deseq2outd)[which(abs(deseq2outd$padj) > 0.01)]
DESeq2::plotMA(res2d)

# TP,FP, Sensitivity, and Specificity Calculations
TPd <- sum(sub.deseq %in% refTPd)
FPd <- sum(!(sub.deseq %in%refTPd))
send <- TPd/length(refTPd)
specd <- FPd/length(sub.deseq)
deseq2.test.results[i,] <- c(size,TPd,FPd,send,specd)
}
